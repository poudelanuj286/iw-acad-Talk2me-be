...
def feed_detail_view(request, feed_id, *args, **kwargs):
    qs = Feed.objects.filter(id=feed_id)
    if not qs.exists():
...
def feed_delete_view(request, feed_id, *args, **kwargs):
    qs = Feed.objects.filter(id=feed_id)
    if not qs.exists():
        return Response({}, status=404)
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def feed_action_view(request, *args, **kwargs):
    '''
    id is required.
    Action options are: like, unlike, share
    '''
    serializer = FeedActionSerializer(data=request.data)
    if serializer.is_valid(raise_exception=True):
        ...
        if action == "like":
            ...
            serializer = FeedSerializer(new_feed)
            return Response(serializer.data, status=200)
...
def feed_create_view_pure_django(request, *args, **kwargs):
    '''
...
def feed_list_view_pure_django(request, *args, **kwargs):
    """
    REST API VIEW
    ...
    """
    qs = Feed.objects.all()
    feeds_list = [x.serialize() for x in qs]
    data = {
        "isUser": False,
        "response": feeds_list
    }
...
def feed_detail_view_pure_django(request, feed_id, *args, **kwargs):
    """
    REST API VIEW
    Consume by JavaScript or Swift/Java/iOS/Andriod
    return json data
